...
class Measoft
{
    ...
    public function calculatorRequest($order = null, $service_mode = '1')
    {
        ...
        if (isset($order['pvzid'])) {
            $xml .= $this->makeXMLNode('calc', '', $level, 'townfrom="' . $order['townfrom'] . '" townto="' . $order['townto'] . '" zipcode="' . $order['zipcode'] . '" pvz="'.$order['pvzid'].'" mass="' . $order['mass'] . '" service="' . $service_mode . '"');
        } else {
            $xml .= $this->makeXMLNode('calc', '', $level, 'townfrom="' . $order['townfrom'] . '" townto="' . $order['townto'] . '" addressto="' . $order['address'] . '" zipcode="' . $order['zipcode'] . '" mass="' . $order['mass'] . '" service="' . $service_mode . '"');
        }        
        $level--;
