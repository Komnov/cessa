...
class ModelExtensionModuleSimilarProducts extends Model {
	...
	public function similarProductsExist($product_id, $data = array()) {
		$sql = "SELECT similar_id FROM " . DB_PREFIX . "product_similar ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.similar_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE ps.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		if ((int)$data['auto_select']) {
			...
			switch ($data['auto_select']) {
				case '1': // By category
					if ($data['categories']) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2c.category_id IN (" . $data['categories'] . ") AND p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
					}
					break;
				...
				case '3': // Model fragment
					if ((int)$data['substr_length']) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.model LIKE CONCAT('%', (SELECT SUBSTR(model, " . ((int)$data['substr_start'] + 1) . ", " . (int)$data['substr_length'] . ") FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'), '%')";
					}
					break;
				case '4': // Name custom string
					if ($data['custom_string']) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "') LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND pd.name LIKE '%" . $this->db->escape($data['custom_string']) . "%'";
					}
					break;
				case '5': // Model custom string
					if ($data['custom_string']) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.model LIKE '%" . $this->db->escape($data['custom_string']) . "%'";
					}
					break;
				case '6': // Product tags
					...
					break;
				case '7': // By manufacturer
					if (isset($data['manufacturer'])) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.manufacturer_id = '" . (int)$data['manufacturer'] . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
					}
					break;
				case '8': // By category & manufacturer
					if ($data['categories'] && isset($data['manufacturer'])) {
						$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2c.category_id IN (" . $data['categories'] . ") AND p.product_id <> '" . (int)$product_id . "' AND p.manufacturer_id = '" . (int)$data['manufacturer'] . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
					}
					break;
				default:
					break;
	...
	public function getSimilarProducts($product_id, $data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE ps.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
			...
			switch ($data['auto_select']) {
				case '1': // By category
					if ($data['categories']) {
						...
						$union_sql .= " WHERE p2c.category_id IN (" . $data['categories'] . ") AND p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
					}
					break;
				case '2': // Name fragment
					if ((int)$data['substr_length']) {
				...
				case '3': // Model fragment
					if ((int)$data['substr_length']) {
						...
						$union_sql .= " WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.model LIKE CONCAT('%', (SELECT SUBSTR(model, " . ((int)$data['substr_start'] + 1) . ", " . (int)$data['substr_length'] . ") FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'), '%')";
					}
					break;
				case '4': // Name custom string
					if ($data['custom_string']) {
				...
				case '5': // Model custom string
					if ($data['custom_string']) {
						...
						$union_sql .= " WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.model LIKE '%" . $this->db->escape($data['custom_string']) . "%'";
					}
					break;
				case '6': // Product tags
					$union_sql = "(SELECT DISTINCT p.product_id" . ($inner_sort_column ? ", " . $inner_sort_column : "") . " FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "') LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
				...
				case '7': // By manufacturer
					if (isset($data['manufacturer'])) {
						$union_sql = "(SELECT p.product_id" . ($inner_sort_column ? ", " . $inner_sort_column : "") . " FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "') LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id <> '" . (int)$product_id . "' AND p.manufacturer_id = '" . (int)$data['manufacturer'] . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND pd.name LIKE '%" . $this->db->escape($data['custom_string']) . "%'";
					}
					break;
				case '8': // By category & manufacturer
					if ($data['categories'] && isset($data['manufacturer'])) {
						...
						$union_sql .= " WHERE p2c.category_id IN (" . $data['categories'] . ") AND p.product_id <> '" . (int)$product_id . "' AND p.manufacturer_id = '" . (int)$data['manufacturer'] . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
					}
					break;
				default:
					break;
		...
		if ($limit > 0 && (int)$start + (int)$data['per_page'] > $limit || !(int)$data['per_page'] && $limit > 0) {
			$count = $limit - $start;
		} else {
		...
		$this->productCount = ($product_count->num_rows) ? (((int)$product_count->row['count'] > $limit && $limit) ? $limit : (int)$product_count->row['count']) : 0;
