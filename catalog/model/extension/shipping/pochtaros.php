<?php
class ModelExtensionShippingPochtaros extends Model {
    private $error = array();
            ...
            if ($status) {
                ...
                if (!empty($region['to']) and (($is_russia == true and ctype_digit($region['to']) and strlen($region['to']) == 6)
                    ...
                    $arr_data = array(
                        ...
                        'to' => $to, 
                        'weight' => $local_weight, 
                        'local_total' => ($local_total != 0) ? $this->convertCurrency($local_total, false, false, true) : 0,
                        'server' => $server,
                    ...
                    $Request = $this->model_localisation_zone_dv->getRequest($arr_data, true);

                    $Request2 = '';
                    if (in_array('p1', $arr_p)) {
                        $Request2 = $this->model_localisation_zone_dv->getRequest($arr_data, false);
                    }
                    ...
                    if ($Request2 and $error == false and $local_total > 0) {
                        $Response2 = $this->cache->get($this->name.'.' . md5($Request2));
                        if (!$Response2) {
                            $curl = curl_init();

                            curl_setopt($curl, CURLOPT_URL, $Request2);
                            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
                            curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 3);
                            ...
                            if ($Response2) {
                                $this->cache->set($this->name.'.' . md5($Request2), $Response2);
                            }
                        ...
                        if (isset($Response2)) {
                            $arrResponse2 = $this->model_localisation_zone_dv->getResponse($Response2);
                        }   
                    }

                    //print_r($arrResponse);
                    //print_r($arrResponse2);
                        ...
                        if (is_array($this->config->get('pochtaros_mstatus')) and count($this->config->get('pochtaros_mstatus')) > 0) {
                            foreach ($this->config->get('pochtaros_mstatus') as $key => $value) {
                                ...
                                if (isset($local['name']) and
                                    ...
                                    isset($arrResponse['Отправления'][$local['name']]['Доставка']) and
                                    ($this->config->get('pochtaros_fragmentation') or 
                                        (!$this->config->get('pochtaros_fragmentation') and
                                            ((isset($arrResponse['Отправления'][$local['name']]['ПредельныйВес']) and 
                                            $weight <= $arrResponse['Отправления'][$local['name']]['ПредельныйВес']) or
                                            !isset($arrResponse['Отправления'][$local['name']]['ПредельныйВес']) )
                                        )
                                    )
                                ) {
                                    ...
                                    if (isset($arrResponse2) and $local['key'] == 'posylka_1_class') {
                                        $price = $this->model_localisation_zone_dv->getPrice($arrResponse2, $local);
                                    }
            ...
            if ((isset($quote_data) and count($quote_data) > 0) or $error) {
                ...
                if ($error and $this->config->get('pochtaros_zaglushka_type') == 1) {
                    ...
                    $method_data = array(
                        ...
                        'sort_order' => ($error & $this->config->get('pochtaros_zaglushka_vniz') == 1) ? ($sort_order + 100) : $sort_order,
                        'error' => ''
                    );
                }
                else {
