...
class ModelLocalisationZoneDv extends Model {
    ...
    public function getRequest($data, $tsennost = true) {
        ...
        if ((isset($data['address']['iso_code_2']) and $data['address']['iso_code_2'] != 'RU') or (isset($data['address']['country_id']) and $data['address']['country_id'] != 176)) {
            if ($local_total > 50000) {
                $local_total = 50000;
            }

            $Request = $url . '?f=' . $data['from'] . '&t=' . $data['address']['iso_code_2'] . '&w=' . $data['weight'] . '&v=' . $local_total . '&o=php&e=0&st=' . $data['server'] . '&ml=' . $this->config->get('config_email') . '&cs=utf-8&pn=' . $this->to_seo($this->config->get('config_owner')) . '&co=' . $data['co'] . '&p=' . $data['p'] . '&key=' . $this->config->get('config_postcalc_key');
        }
        else {
    ...
    public function getPrice($arrResponse, $local) {
        ...
        if (!$price) {
            $price = floor($arrResponse['Отправления'][$local['name']]['Тариф']);
        }
