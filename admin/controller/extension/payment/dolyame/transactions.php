...
class ControllerExtensionPaymentDolyameTransactions extends Controller
{
	...
	{
		if (!$this->user->hasPermission('access', 'extension/payment/dolyame/transactions')) {
		...
		foreach ($results as $result) {
			...
			if ($result['status'] == 'committed' && $this->user->hasPermission('modify', 'extension/payment/dolyame/transactions')) {
	...
	{
		if (!$this->user->hasPermission('modify', 'extension/payment/dolyame/transactions')) {
			return new Action('error/permission');
		}
		$this->load->language('extension/payment/dolyame/transactions');
